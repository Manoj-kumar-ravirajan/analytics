WITH city_cluster_hex AS (
    ...
    orders AS (
        ...
        SELECT
            oli.yyyymmdd AS yyyymmdd,
            oli.service_detail_id AS service_detail_id,
            oli.estimate_id AS fare_estimate_id,
            oli.order_status AS order_status,
            oli.order_id AS order_id,
            row_number() over (partition by oli.order_id order by oli.updated_epoch desc) as rank_1,
            CASE 
            WHEN oli.order_status in ('dropped') and oli.spd_fraud_flag != True then 'Net_Orders'
            WHEN oli.order_status in ('customerCancelled') and oli.cancel_reason in ('order cancelled before rider accepted') then 'COBRA'
            WHEN oli.order_status in ('customerCancelled') and oli.cancel_reason in ('Order cancelled before rider was mapped') then 'COBRM'
            WHEN oli.order_status in ('customerCancelled') and oli.cancel_reason not in ('order cancelled before rider accepted', 'Order cancelled before rider was mapped') then 'OCARA'
            ELSE 'Other' END AS end_state
        FROM
            orders.order_logs_immutable oli
        WHERE
            oli.yyyymmdd >= DATE_FORMAT(DATE('{{StartDate}}'), '%Y%m%d') 
            AND oli.yyyymmdd <= DATE_FORMAT(DATE('{{EndDate}}'), '%Y%m%d')
            AND oli.service_obj_service_name IN ('Link', 'Auto', 'Auto Pool', 'CabEconomy')
    ),
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN cdf.end_state = 'OCARA' AND cdf.rank_1 = 1 THEN cdf.order_id END) AS ocara_orders
FROM
    combined_df cdf
GROUP BY
    geo_level,
