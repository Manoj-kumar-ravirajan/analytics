...
(
    ...
    and yyyymmdd between date_format(CURRENT_DATE - interval '7' day,'%Y%m%d') and date_format(CURRENT_DATE - interval '1' day,'%Y%m%d')
),
order_coin as
(
    select 
...
(
    ...
    where offer_start_date = date_format(date_trunc('week',current_date-interval '1' day), '%Y-%m-%d')
...
(
    ...
    AND CAST(day AS DATE) BETWEEN (DATE_TRUNC('week',CURRENT_DATE) - INTERVAL '21' day)
