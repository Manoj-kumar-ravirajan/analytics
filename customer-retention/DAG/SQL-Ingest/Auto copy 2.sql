...
(
    ...
    where
    cast(day as date) between DATE_TRUNC('week',{{start_date}}) - interval '7' day and {{end_date}} - interval '1' day
...
(
    ...
    and yyyymmdd between date_format({{start_date}} - interval '7' day,'%Y%m%d') and date_format({{end_date}} - interval '1' day,'%Y%m%d')
),
order_coin as
(
    select 
    ...
    where 1=1
    and yyyymmdd between date_format(DATE_TRUNC('week',{{start_date}}) - interval '7' day,'%Y%m%d') and date_format({{end_date}} - interval '1' day,'%Y%m%d')
...
(
    ...
    where offer_start_date = date_format(date_trunc('week',{{start_date}}-interval '1' day), '%Y-%m-%d')
    and run_date =  date_format(date_trunc('week',{{start_date}}-interval '1' day)-interval '3' day, '%Y-%m-%d')
    and element_at(offer_service,1) = 'AUTO'
),
...
(
    ...
    AND CAST(day AS DATE) BETWEEN (DATE_TRUNC('week',{{start_date}}) - INTERVAL '21' day)
...
(
    select
        ...
        DPR,
        Fe_RR,
        coalesce(allocator_status,'Inactive') Allocator_status
...
select
    ...
    EndDate as enddate,
    city,
    Allocator_status as allocator_status,
