...
WHERE
(yyyymmdd >= date_format({{main_start_date}}, '%Y%m%d') AND yyyymmdd <= date_format({{end_date}}, '%Y%m%d'))
AND service_name in ('Link','Auto')

AND current_city = 'Chennai' --IN ('Hyderabad', 'Delhi', 'Chennai')
...
segment as (
select 
    week(cast(run_date as date)+ INTERVAL '1' DAY) as wk,
    -- taxi_lifetime_last_ride_city AS city,
    customer_id,
    link_lifetime_stage AS lifetime_stage_link,
    auto_lifetime_stage AS lifetime_stage_auto,
    taxi_lifetime_stage AS lifetime_stage_taxi
from datasets.iallocator_customer_segments s
where
    run_date between DATE_FORMAT(DATE_TRUNC('week', {{main_start_date}}) - INTERVAL '1' DAY, '%Y-%m-%d')
    and DATE_FORMAT(DATE_TRUNC('week', {{end_date}} + INTERVAL '7' DAY) - INTERVAL '2' DAY, '%Y-%m-%d') 
    -- AND taxi_lifetime_last_ride_city in (select distinct city from base)
    and day_of_week(cast (run_date as date)) = 7
    and customer_id IN (select distinct customer_id from fe_tbl)
),
...
WHERE
(cast(yyyymmdd as varchar) >= date_format({{main_start_date}}, '%Y%m%d') AND yyyymmdd <= date_format({{end_date}}, '%Y%m%d'))
...
segment.lifetime_stage_link,
segment.lifetime_stage_auto,
segment.lifetime_stage_taxi,
...
LEFT JOIN 
segment
ON fe_merged.customer_id = segment.customer_id
AND fe_merged.wk = segment.wk
...
v0 as
(
select city, week_start_date, coalesce(lifetime_stage_auto, 'NEW') lifetime_stage_auto,
...
from final_v0
group by city, week_start_date,lifetime_stage_auto
order by city, week_start_date,lifetime_stage_auto
)
