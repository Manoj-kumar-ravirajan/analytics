...
SELECT 
    ...
    customer_id,
    CASE
    WHEN link_lifetime_rides IS NULL THEN 'NEW'
    WHEN link_lifetime_rides BETWEEN 1 AND 4 THEN '1-4'
    WHEN link_lifetime_rides >= 5 THEN '>=5'
    ELSE 'NEW'
    END ltr_link,
    CASE 
    WHEN link_lifetime_last_ride_date IS NULL THEN 'LTR=0'
    ...
    WHEN link_lifetime_last_ride_date < (DATE_TRUNC('month', DATE(month)) - INTERVAL '90' DAY) THEN '3. >90_DAYS'
    ELSE 'LTR=0'
    END recency_link
...
SELECT 
        ...
        COALESCE(b.recency_link, 'LTR=0') recency_link,
